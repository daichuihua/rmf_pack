...
# States for RobotCommandHandle's state machine used when guiding robot along a new path
...
class RobotCommandHandle(adpt.RobotCommandHandle):
        ...
        def _follow_path():
            ...
            while (
                ...
                if self.state == RobotState.IDLE:
                    ...
                    x,y,theta = self.api.transform_rmf_to_robot(self.name,target_pose)
                    ...
                    with self._lock:
                        ...
                        if lane is not None:
                            ...
                            if self.target_waypoint.graph_index is not \
                                None and self.dist(self.position, target_pose) < 0.4:
                                ...
                                self.position, self.graph.get_waypoint(
                                    self.last_known_waypoint_index).location) < 0.4:
    ...
    def get_position(self):
        ...
        if position is not None:
            # print(f'44444444----------------Update RMF get_position :{position[0]},{position[1]},{position[2]}','时间：',ctime())
            x,y,theta = self.api.transform_robot_to_rmf(self.name,position)
            # print(f'CommandHandle get_position  Transform:{x},{y},{theta}')
    ...
    def update(self):
        self.node.get_logger().info("Update_on_myway.")
    ...
    def dist(self, A, B):
        ...
        assert(len(A) > 1)
        assert(len(B) > 1)
        ret = math.sqrt((A[0] - B[0])**2 + (A[1] - B[1])**2)
        print(f"dist :{ret}")
        return ret
